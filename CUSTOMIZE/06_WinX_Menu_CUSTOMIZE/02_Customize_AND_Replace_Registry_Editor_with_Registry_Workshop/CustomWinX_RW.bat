...
:: Import
	...
	"%ProgramFiles%\7-Zip\7z.exe" x "%~dp0CustomWinX_RW.zip" -y -r -o"%LocalAppdata%\Microsoft\Windows\WinX" >NUL & goto :RestoreAttrib
	) || (
	goto WINRAR
	)
:WINRAR
	if exist "%programFiles%\WinRAR\WinRAR.exe" (
	"%programFiles%\WinRAR\WinRAR.exe" x "%~dp0CustomWinX_RW.zip" -ibck -o+ "%LocalAppdata%\Microsoft\Windows\WinX" >NUL & goto :RestoreAttrib
	) || (
	goto PS
	)
:PS
	PowerShell -NoProfile -ExecutionPolicy Bypass "Expand-Archive -Path "CustomWinX_RW.zip" -DestinationPath "$env:LocalAppdata\Microsoft\Windows\WinX" -Force" >NUL 2>&1
:RestoreAttrib
