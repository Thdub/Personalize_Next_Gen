...
:: Check Directories
	...
	if exist "%LocalAppdata%\Microsoft\Windows\WinX\Group1\2 - Lock.lnk" goto :Import
	if exist "%LocalAppdata%\Microsoft\Windows\WinX\Group1\" rmdir "%LocalAppdata%\Microsoft\Windows\WinX\Group1" /s /q >NUL 2>&1
:Import
	cd /d "%~dp0"
	if exist "%ProgramFiles%\7-Zip\7z.exe" (
	"%ProgramFiles%\7-Zip\7z.exe" x "%~dp0SoftRestart.zip" -y -r -o"%LocalAppdata%\Microsoft\Windows\WinX" >NUL & goto :RestoreAttrib
	) || (
	goto WINRAR
	)
:WINRAR
	if exist "%programFiles%\WinRAR\WinRAR.exe" (
	"%programFiles%\WinRAR\WinRAR.exe" x "%~dp0SoftRestart.zip" -ibck -o+ "%LocalAppdata%\Microsoft\Windows\WinX" >NUL & goto :RestoreAttrib
	) || (
	goto PS
	)
:PS
	PowerShell -NoProfile -ExecutionPolicy Bypass "Expand-Archive -Path "SoftRestart.zip" -DestinationPath "$env:LocalAppdata\Microsoft\Windows\WinX" -Force" >NUL 2>&1
:RestoreAttrib
